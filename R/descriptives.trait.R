"descriptives.trait" <-
function (data,subset,file,by=NULL,digits=3) {
	...
	if (!is.null(by)) {
		svar <- by
		if (length(unique(svar))!=2) stop("The by argument should contain a binary variable")
	...
	if (is.null(by)) 
		...
		colnames(out) <- c(paste("No(by=",lvls[1],")",sep=""),"Mean","SD",paste("No(by=",lvls[2],")",sep=""),"Mean","SD","Ptt","Pkw","Pexact")
	if (!missing(file)) {
		cat("\t",file=file,sep="")
