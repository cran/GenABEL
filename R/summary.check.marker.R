...
function(object,...) {
	...
	out2[1,1] <- length(object$idnocall) - out2[1,2] - out2[1,3] - out2[1,4] - out2[1,4] - out2[1,5] - out2[1,6]
	...
	out2[2,2] <- length(object$hetfail) - out2[1,2] - out2[2,3] - out2[2,4] - out2[2,4] - out2[2,5] - out2[2,6]
	...
	out2[3,3] <- length(object$ibsfail) - out2[1,3] - out2[2,3] - out2[3,4] - out2[3,4] - out2[3,5] - out2[3,6]
	out2[4,5] <- cross(object$isfemale,object$ismale)
	out2[4,6] <- cross(object$isfemale,object$isXXY)
	out2[4,4] <- length(object$isfemale) - out2[1,4] - out2[2,4] - out2[3,4] - out2[4,5] - out2[4,6]
	out2[5,6] <- cross(object$ismale,object$isXXY)
	out2[5,5] <- length(object$ismale) - out2[1,5] - out2[2,5] - out2[3,5] - out2[4,5] - out2[5,6]
	out2[6,6] <- length(object$isXXY) - out2[1,6] - out2[2,6] - out2[3,6] - out2[4,6] - out2[5,6]
#	rownames(out2) <- c("IDnoCall","HetFail","IBSFail","Xidfail","isfemale","ismale")
#	rownames(out2) <- c("IDnoCall","HetFail","IBSFail","isfemale","ismale")
	rownames(out2) <- c("IDnoCall","HetFail","IBSFail","isfemale","ismale","isXXY")
